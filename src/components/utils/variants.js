const variants = ( product, currentVariant ) => {
    ...
    product.variants?.forEach( variant => {
        const variantButton = `
            <li class="mb-0">
                <button class="varient-button d-block rounded-circle transition-3 ${ variant.id === currentVariant ? 'active' : null }" style="--variant-base-color: ${variant.colors[ 0 ]};background-image: linear-gradient( -135deg, ${
