import{v as E,h as S,c as $,d as g,g as D,a as b,b as u,e as v,m as H,x as A}from"./alert-DMWuZ5g4.js";const f=()=>new Promise(e=>{E(S,async t=>{e(t)})}),M=async()=>{try{const e=await f();if(e){const t=$(g,"users",e.uid,"cart");return(await D(t)).size}else return(JSON.parse(localStorage.getItem("guestCart"))||[]).length}catch(e){console.log(e)}},k=document.querySelector("#mainHeader .navbar-nav #cart .cart-badge"),y=async()=>{try{const e=await M();k&&(k.textContent=e)}catch(e){console.err(e)}},x=(e,t,s)=>{const a=e.variants.find(c=>c.id===t),r=a==null?void 0:a.images[0];return`
    ...
    `,r=document.createElement("div");r.id="cartDrawerSkeleton";for(let c=0;c<t;c++)r.insertAdjacentHTML("beforeend",a());s.appendChild(r)},I=document.getElementById("cart"),n=document.getElementById("cartProducts");function m(){const e=document.getElementById("cartDrawerSkeleton");e&&e.remove()}const B=async()=>{O(n,5);const e=await f();if(e){const t=$(g,"users",e.uid,"cart"),s=await D(t);if(s.empty)n&&n.insertAdjacentHTML("beforeend",b("Your Cart is Empty")),m();else for(const a of s.docs){const r=a.data(),{productId:c,variantId:o,quantity:i}=r;try{const l=u(g,"collections","products","items",c),d=await v(l);if(d.exists()){const p=d.data();n&&n.insertAdjacentHTML("afterbegin",x(p,o,i))}}catch(l){console.log(l)}finally{m()}}}else{const t=JSON.parse(localStorage.getItem("guestCart"))||[];if(t.length===0)n&&n.insertAdjacentHTML("beforeend",b("Your Cart is Empty")),m();else for(const s of t){const{productId:a,variantId:r,quantity:c}=s;try{const o=u(g,"collections","products","items",a),i=await v(o);if(i.exists()){const l=i.data();n&&n.insertAdjacentHTML("beforeend",x(l,r,c))}}catch(o){console.log(o)}finally{m()}}}requestAnimationFrame(()=>{document.querySelectorAll(".delete-cart-item").forEach(s=>{s.addEventListener("click",async a=>{var i,l;a.preventDefault();const r=s.getAttribute("data-product-id"),c=s.getAttribute("data-variant-id");s.disabled=!0;const o=await f();if(o)await H(u(g,"users",o.uid,"cart",`${r}_${c}`)),y(),(i=s.closest(".cart-item"))==null||i.remove();else{let d=JSON.parse(localStorage.getItem("guestCart"))||[];d=d.filter(p=>!(p.productId===r&&p.variantId===c)),localStorage.setItem("guestCart",JSON.stringify(d)),y(),(l=s.closest(".cart-item"))==null||l.remove()}})})})};I&&I.addEventListener("click",e=>{e.preventDefault(),B()});const j=async()=>{try{await A(S),window.location.replace("./login.html")}catch(e){console.log(e)}},C=document.getElementById("app"),L=document.getElementById("currentUser"),w=document.getElementById("mainHeader"),T=w.clientHeight;C&&(C.style.paddingTop=`${T}px`);w&&w.classList.add("position-fixed","top-0","end-0","start-0","z-3");const N=async()=>{const e=await f();if(e)try{const t=u(g,"users",e.uid),s=await v(t);if(s.exists()){const a=s.data();L.innerHTML=`
    ...
    `},q=()=>{const e=document.getElementById("logOut");e&&e.addEventListener("click",t=>{t.preventDefault(),j()})};N().then(y);export{f as g,j as h,y as u};
