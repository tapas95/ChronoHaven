import{l as k,h as y,d as p,u as S,c as q,g as A,k as I}from"./firebase-config-D-A2ZIku.js";import{g as v,u as $}from"./header-JauWKcJv.js";import{d as C}from"./alert-CO0lNkg5.js";const w=(e,t,s)=>{const a=e.variants.find(r=>r.id===t),n=a==null?void 0:a.images[0],i=a==null?void 0:a.colors[0];return`
    ...
    `,n=document.createElement("div");n.className="row d-flex row-gap-4 px-0 mx-0",n.id="cartItemsSkeleton";for(let i=0;i<t;i++)n.insertAdjacentHTML("beforeend",a());s.appendChild(n)},o=document.getElementById("cartItems"),g=document.getElementById("subTotal"),f=document.getElementById("total");function b(){const e=document.getElementById("cartItemsSkeleton");e&&e.remove()}const h=async()=>{E(o,3);const e=await v();let t=0;if(e){const s=q(p,"users",e.uid,"cart"),a=await A(s);if(a.empty)o&&o.insertAdjacentHTML("beforeend",C("Your Cart is Empty","danger","bi-exclamation-diamond-fill","mt-0")),b();else for(const n of a.docs){const i=n.data(),{productId:r,variantId:c,quantity:l}=i;try{const d=y(p,"collections","products","items",r),u=await I(d);if(u.exists()){const m=u.data();t+=m.price.current*l,o&&o.insertAdjacentHTML("beforeend",w(m,c,l))}}catch(d){console.log(d)}finally{b()}}requestAnimationFrame(()=>{g&&(g.textContent=`₹ ${t.toFixed(2)}`),f&&(f.textContent=`₹ ${t.toFixed(2)}`)})}else{const s=JSON.parse(localStorage.getItem("guestCart"));if(s.length===0)o&&o.insertAdjacentHTML("beforeend",C("Your Cart is Empty","danger","bi-exclamation-diamond-fill","mt-0")),b();else for(const a of s){const{productId:n,variantId:i,quantity:r}=a;try{const c=y(p,"collections","products","items",n),l=await I(c);if(l.exists()){const d=l.data();t+=d.price.current*r,o&&o.insertAdjacentHTML("beforeend",w(d,i,r))}}catch(c){console.log(c)}finally{b()}}requestAnimationFrame(()=>{g&&(g.textContent=`₹ ${t.toFixed(2)}`),f&&(f.textContent=`₹ ${t.toFixed(2)}`)})}};h();o&&o.addEventListener("click",async e=>{const t=e.target.closest(".delete-cart-item");if(!t)return;const s=t.getAttribute("data-product-id"),a=t.getAttribute("data-variant-id"),n=await v();if(n)t.disabled=!0,await k(y(p,"users",n.uid,"cart",`${s}_${a}`)),h(),$();else{t.disabled=!0;let i=JSON.parse(localStorage.getItem("guestCart"))||[];i=i.filter(r=>!(r.productId===s&&r.variantId===a)),localStorage.setItem("guestCart",JSON.stringify(i)),h(),$()}});o&&o.addEventListener("click",async e=>{var l,d;const t=e.target.closest(".add-quantity"),s=e.target.closest(".remove-quantity"),a=(l=t||s)==null?void 0:l.getAttribute("data-product-id"),n=(d=t||s)==null?void 0:d.getAttribute("data-variant-id");if(!a||!n)return;const i=document.getElementById(`${a}_${n}`);if(!i)return;let r=parseInt(i.value)||1;t&&(t.disabled=!0),s&&(s.disabled=!0),t&&r<10&&r++,s&&r>1&&r--,i.value=r;const c=await v();if(c)await S(y(p,"users",c.uid,"cart",`${a}_${n}`),{quantity:r});else{let u=JSON.parse(localStorage.getItem("guestCart"))||[];const m=u.findIndex(x=>x.productId===a&&x.variantId===n);m!==-1&&(u[m].quantity=r,localStorage.setItem("guestCart",JSON.stringify(u))),console.log(m)}h()});
